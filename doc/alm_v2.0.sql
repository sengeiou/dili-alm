...
CREATE TABLE `project_apply` (
  ...
  COMMENT '产品经理',
  `development_manager`  INT          NULL
  COMMENT '研发经理',
  `test_manager`         INT          NULL
  COMMENT '测试经理',
  `business_owner`       INT          NULL
  COMMENT '业务负责人',
  `dep`                  INT          NULL
  COMMENT '所属部门',
  ...
  `create_member_id`     INT          NULL,
  `modify_member_id`     INT          NULL,
  PRIMARY KEY (`ID`)
);

CREATE TABLE `approve` (
  `id`               INT ZEROFILL NOT NULL AUTO_INCREMENT,
  `project_apply_id` INT          NULL,
  `status`           TINYINT      NULL,
  `description`      VARCHAR(255) NULL,
  `type`             TINYINT      NULL
  COMMENT '审批类型（立项，结项）',
