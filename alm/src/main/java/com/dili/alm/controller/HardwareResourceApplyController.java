...
public class HardwareResourceApplyController {
	...
	public String updateView(@RequestParam Long id, ModelMap modelMap) throws HardwareResourceApplyException {
		...
		List<String> envList = (List<String>) JSONObject.parseObject(dto.getServiceEnvironment(), new TypeReference<List<String>>() {
		});
		...
		List<HardwareResourceRequirementDto> requirementDtoList = new ArrayList<HardwareResourceRequirementDto>(requirementList.size());
		requirementList.forEach(c->{requirementDtoList.add(DTOUtils.toEntity(c, HardwareResourceRequirementDto.class, true));});
...
/*    	@SuppressWarnings("rawtypes")
    ...
    public @ResponseBody BaseOutput managerApprove(Long id,boolean isApproved,String description) throws HardwareResourceApplyException {
    ...
    public @ResponseBody BaseOutput generalManagerApprove(Long id,boolean isApproved,String description) throws HardwareResourceApplyException {
    	UserTicket user = SessionContext.getSessionContext().getUserTicket();
    	if (isApproved) {
    ...
    public @ResponseBody BaseOutput operManagerApprove(Long id,boolean isApproved,String description,String[]executorsChk) throws HardwareResourceApplyException {
    	UserTicket user = SessionContext.getSessionContext().getUserTicket();
