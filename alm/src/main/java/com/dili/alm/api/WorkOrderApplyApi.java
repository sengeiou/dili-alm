...
public class WorkOrderApplyApi {
	...
	public @ResponseBody BaseOutput<Assignment> setOrderApplyAssigneeName(@RequestBody TaskMapping taskMapping) {
		/*
		 * Map<String, String[]> map = request.getParameterMap(); String[] strs =
		 * map.get("processVariables"); JSONObject jsonObj = JSON.parseObject(strs[0]);
		 * Assignment record
		 * =workOrderApply.setOrderApplyName(jsonObj.getLong("businessKey"));
		 */
	...
	public @ResponseBody BaseOutput<Assignment> setAllocateAssigneeName(TaskMapping taskMapping) {
		 ...
		 * map.get("processVariables"); JSONObject jsonObj = JSON.parseObject(strs[0]);
		 * Assignment record
		 * =workOrderApply.setAllocateName(jsonObj.getLong("businessKey"));
		 */
