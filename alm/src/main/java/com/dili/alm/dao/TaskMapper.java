...
public interface TaskMapper extends MyMapper<Task> {
	...
	List<TaskByUsersDto> selectTaskHourByUser(@Param("beginTime") String beginTime, @Param("endTime") String endTime, @Param("departmentIds") List<Long> dids, @Param("uIds") List<Long> uids,
			@Param("order") String order, @Param("sort") String sort, @Param("users") Collection<User> users, @Param("depts") Collection<Department> depts);
	...
	List<SelectTaskHoursByUserProjectDto> selectUsersProjectHours(@Param("userIds") List<Long> userId, @Param("projectIds") List<Long> projectId, @Param("startTime") Date startTime,
