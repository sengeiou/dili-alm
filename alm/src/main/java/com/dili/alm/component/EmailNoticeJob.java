...
public class EmailNoticeJob implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void scan(ScheduleMessage scheduleMessage) {
		...
		// JSONObject.parseObject(scheduleMessage.getJobData(), ProjectVersion.class);
		...
		// this.projectMapper.getChildProjects(projectVersion.getProjectId());
		...
		// log.error("MessagingException, 邮件发送失败, email:" +
		// AlmCache.userMap.get(team.getMemberId()).getEmail());
		// e.printStackTrace();
		// log.error(e.getMessage());
		// return;
		// } catch (Exception e) {
		// log.error("Exception, 邮件发送失败, email:" +
		// AlmCache.userMap.get(team.getMemberId()).getEmail());
		// e.printStackTrace();
		// log.error(e.getMessage());
		// return;
